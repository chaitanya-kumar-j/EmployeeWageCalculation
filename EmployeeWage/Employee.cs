...
namespace EmployeeWage
{
    class Employee
    {
        ...
        {
            // Define constants
            const int IS_FULL_TIME = 1, IS_PART_TIME = 2, FULL_DAY_HRS = 8, PART_TIME_HRS = 4, WAGE_PER_HR = 20, MAX_WORKING_DAYS = 20;
            // Define variables
            int empHours = 0;
            // Create random number generator
            Random random = new Random();
            // Calculate Total hours worked in a month
            for (int workingDays = 1; workingDays < MAX_WORKING_DAYS; workingDays++)
            {
